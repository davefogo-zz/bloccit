...
RSpec.describe Api::V1::UsersController, type: :controller do
  ...
  context "unauthenticated users" do
    ...
    it "POST create returns http unauthenticated" do
      new_user = build(:user)
      post :create, { name: new_user.name, email: new_user.email, password: new_user.password }
      expect(response).to have_http_status(401)
    end
  end
  ...
  describe "PUT update" do
    ...
    context "with invalid attributes" do
      ...
      it "returns the correct json error message" do
        expect(response.body).to eq({ error: "User update failed", satus: 400}.to_json)
      end
    end
  end
  ...
  describe "POST create" do
    context "with valid attributes" do
      ...
      it "creates a user with the correct attributes" do
        ...
        expect(hashed_json["name"]).to eq(@new_user.name)
        expect(hashed_json["email"]).to eq(@new_user.email)
        exepct(hashed_json["role"]).to eq("admin")
      end
    end
    ...
    context "with invalid attributes" do
      ...
      it "returns the correct json error message" do
        expect(response.body).to eq({"error": "User is invalid", "satus": 400}.to_json)
      end
    end
  end
