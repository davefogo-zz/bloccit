...
RSpec.describe SponsoredPostsController, type: :controller do
  ...
  describe "GET #edit" do
    ...
    it "assigs the sponsored_post to be updated to @sponsored_post " do
      get :edit, {topic_id: my_topic.id, id: my_sponsored_post.id}
      updated_instance = assigns(:sponsored_post)

      expect(updated_instance.id).to eq(my_sponsored_post.id)
      expect(updated_instance.title).to eq(my_sponsored_post.title)
      expect(updated_instance.body).to eq(my_sponsored_post.body)
      expect(updated_instance.price).to eq(my_sponsored_post.price)
    end
  end
